...
int main()
{
    ...
    while(min_vertex < max_vertex)
    {
        ...
        for (int i = 0; i < test_count; ++i)
        {
            ...
            standard_deviation += (ans - correct_ans) * (ans - correct_ans);  //подсчет абсолютная погрешности
            mean_value += ans / correct_ans;   //подсчет относительной погрешности
        }
        cout << "Quantity of vertices = " << min_vertex << endl;
        cout << "Standard deviation = " << sqrt(standard_deviation / test_count) << endl; //абсолютная, для информации
        cout << "Mean relative deviation = " << mean_value / test_count << endl; //относительная, ннужно проверить, что 1.5
        cout << endl;
        ++min_vertex;
    }
    return 0;
